...
public class NetworkPlayerController : MonoBehaviourPunCallbacks
{
    ...
    public float ledgeGrabRange = 0.125f;
    ...
    public NetworkPlayerMovement movement;
    ...
    void CheckLedgeGrab()
    {
        ...
        bool right = Physics.Raycast(pos + (transform.right * radius / 2f), dir, ledgeGrabRange, ledgeLayer);
        bool left = Physics.Raycast(pos - (transform.right * radius / 2f), dir, ledgeGrabRange, ledgeLayer);

        if (Physics.Raycast(pos, dir, out var hit, ledgeGrabRange, ledgeLayer) && right && left)
        {
